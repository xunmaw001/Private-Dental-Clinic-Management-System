...
<body>
	<div id="main-container">
		...
		<div id="breadcrumb-container">
			<h3 class="breadcrumb-title">编辑病例信息</h3>
			<ol class="breadcrumb-list">
				...
				<li class="breadcrumb-item-one"><span>病例信息管理</span></li>
				<li class="breadcrumb-item-two"><span>编辑病例信息</span></li>
			</ol>
		</div>
		...
		<div id="add-container">
			<form id="addOrUpdateForm">
				...
				<div class="form-item">
					<label>病例名称</label>
					<input name="binglimingcheng" id="binglimingcheng" name="binglimingcheng" class="form-control" placeholder="请输入病例名称">
				</div>
				<div class="form-item">
					<label>病例类型</label>
					<select id="binglileixingSelect" name="binglileixing" class="form-control">
							<option value="">请选择病例类型</option>
							<option class="binglileixingOption" value="历史病例">
								历史病例
							</option>
							<option class="binglileixingOption" value="新增病例">
								新增病例
							</option>
					</select>	
				</div>	
				...
				<div class="form-item-file">
					<label>病例视频</label>
					<div class="upload btn-file">选择文件<input name="file" type="file" class="form-control-file" id="binglishipinupload"></div>
					<div id="binglishipinFileName" class="text"></div>
					<input name="binglishipin" id="binglishipin-input" type="hidden">
				</div>
				<div class="form-item">
					<label>发布时间</label>
				...
				<div class="form-item-file" >
					...
					<div class="upload btn-file">
						插入图片
						<input id="binglijiangjieupload" name="file" type="file">
					</div>
					<script id="binglijiangjieEditor" type="text/plain"></script>
					<script type="text/javascript">
						//建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
					   //相见文档配置属于你自己的编译器
					 var binglijiangjieUe = UE.getEditor('binglijiangjieEditor', {
						toolbars: [
							[
								'undo', //撤销
								'bold', //加粗
					...
					<input type="hidden" name="binglijiangjie" id="binglijiangjie-input">
				</div>
				<div class="form-btn">
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "binglixinxi/info/" + id,
		...
		function upload(){
			...
			$('#binglishipinupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {				
					document.getElementById('binglishipin-input').setAttribute('value',"upload/"+data.result.file);
					if(document.getElementById('binglishipinFileName') != null){
						document.getElementById('binglishipinFileName').innerHTML = "上传成功!";
					}				
					$("#binglishipinImg").attr("src",baseUrl+"upload/"+data.result.file);												
				}
			});
			$('#binglijiangjieupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('binglijiangjieEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
		...
		function submit() {
               //获取表单数据前取消禁用
                $("#fabushijian-input").attr('disabled',false);
				...
				if(crossrefid && crossuserid) {
					...
					http("binglixinxi/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("binglixinxi/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.binglijiangjie != null && ruleForm.binglijiangjie != 'null' && ruleForm.binglijiangjie != ''){
				var binglijiangjieMes = '' + ruleForm.binglijiangjie;
				var binglijiangjieUeditor = UE.getEditor('binglijiangjieEditor');
				binglijiangjieUeditor.ready(function() {
					binglijiangjieUeditor.setContent(binglijiangjieMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('binglijiangjieEditor').hasContents()){
				$('#binglijiangjie-input').attr('value',UE.getEditor('binglijiangjieEditor').getContent());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("binglixinxi/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function dataBind(){
			...
			var binglileixingOptions = document.getElementsByClassName("binglileixingOption");
			for(var binglileixingCount = 0; binglileixingCount < binglileixingOptions.length;binglileixingCount++){
				if(binglileixingOptions[binglileixingCount].getAttribute('value') == ruleForm.binglileixing){
					binglileixingOptions[binglileixingCount].setAttribute('selected','selected');
				}
			}
            if(ruleForm.tupian){
			...
			$("#binglishipin-input").val(ruleForm.binglishipin);
			$("#binglishipinFileName").html(ruleForm.binglishipin);
			$("#binglijiangjie").val(ruleForm.binglijiangjie);	
			...
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.binglibianhao != null && res.data.binglibianhao != ''){
						...
						$("#binglibianhao").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.binglimingcheng != null && res.data.binglimingcheng != ''){
						...
						$("#binglimingcheng").attr('readonly','readonly');
					}
					if(res.data != null && res.data != '' && res.data.binglileixing != null && res.data.binglileixing != ''){

						var binglileixingOptions = document.getElementsByClassName("binglileixingOption");
						for(var binglileixingCount = 0; binglileixingCount < binglileixingOptions.length;binglileixingCount++){
							if(binglileixingOptions[binglileixingCount].getAttribute('value') == res.data.binglileixing){
								binglileixingOptions[binglileixingCount].setAttribute('selected','selected');
							}
						}
					}
					...
					if(res.data != null && res.data != '' && res.data.binglishipin != null && res.data.binglishipin != ''){

						$("#binglishipin-input").val(res.data.binglishipin);
						$('#binglishipinFileName').val(res.data.binglishipin)
					}
					if(res.data != null && res.data != '' && res.data.binglijiangjie != null && res.data.binglijiangjie != ''){

						$("#binglijiangjie").val(res.data.binglijiangjie);
                        var binglijiangjieMes = '' + res.data.binglijiangjie;
                        var binglijiangjieUeditor = UE.getEditor('binglijiangjieEditor');
                        binglijiangjieUeditor.ready(function() {
                            binglijiangjieUeditor.setContent(binglijiangjieMes);
                        });
		...
		function readonly(){
            if(window.sessionStorage.getItem('role')!="管理员") {
                ...
                $("#fabushijian-input").attr('disabled',true);
            }
		}
