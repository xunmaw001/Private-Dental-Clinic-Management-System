			...
			<div class="recommend" :style='{"padding":"0","margin":"0px auto","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative"}'>
				...
				<form class="layui-form filter" :style='{"padding":"60px 7% 0","alignItems":"center","background":"none","display":"flex","width":"100%","justifyContent":"center","height":"auto"}'>
					...
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>病例名称</div>
						<input type="text" :style='{"border":"1px solid #eee","padding":"0 10px","boxShadow":"0px 0px 0px #ccc","margin":"0","outline":"none","color":"#555","borderRadius":"0px","width":"140px","fontSize":"14px","lineHeight":"40px","height":"40px"}' name="binglimingcheng" id="binglimingcheng" placeholder="病例名称" autocomplete="off" class="layui-input">
					</div>
					<div :style='{"alignItems":"center","margin":"0 4px 0 0","display":"flex"}' class="item-list">
						<div class="lable" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px"}'>病例类型</div>
						<select name="binglileixing" id="binglileixing" lay-filter="binglileixing">
							<option value="">请选择</option>
							<option v-for="(item,index) in binglileixingOptions" v-bind:key="index" :value="item">{{item}}</option>
						</select>
					</div>
		...
		<script>
			...
			layui.use(['form', 'layer', 'element', 'carousel', 'laypage', 'http', 'jquery','laydate', 'slider'], function() {
				...
				http.request('config/list', 'get', {
			        ...
			        vue.binglileixingOptions = '历史病例,新增病例'.split(',')
				...
				function pageList() {
			        ...
			        if (jquery('#binglimingcheng').val()) {
			          param['binglimingcheng'] = jquery('#binglimingcheng').val() ? '%' + jquery('#binglimingcheng').val() + '%' : '';
			        }
			        if (jquery('#binglileixing').val()) {
			          param['binglileixing'] = jquery('#binglileixing').val() ? jquery('#binglileixing').val() : '';
			        }
				...
				if (jquery('#binglileixing').val()) {
					...
					// 获取列表数据
					http.request('binglixinxi/list', 'get', param, function(res) {
